...
// Skips automatically if the server is not reachable or unhealthy.
//
// Run:
//
//	go test -v -timeout 60m ./...
//
// Reports land in testdata/RESULTS_HTTP_TOOL_USE_<timestamp>.md.
...
type Config struct {
	...
	CloudCtlURL string        `yaml:"cloud_ctl_url"`
...
func loadConfig(t *testing.T) Config {
	...
	if err != nil {
		b, err = os.ReadFile(filepath.Join("22_cloud_ctl_http_tool_use", "config.yaml"))
		if err != nil {
	...
	if cfg.CloudCtlURL == "" {
		cfg.CloudCtlURL = "http://localhost:8085"
	}
	return cfg
}
...
func loadTestMatrix(t *testing.T) []TestCase {
	...
	if err != nil {
		b, err = os.ReadFile(filepath.Join("22_cloud_ctl_http_tool_use", "tests.yaml"))
		if err != nil {
...
// =============================================================================
// Cloud-CTL server availability check
// =============================================================================

// checkServerAvailable verifies the cloud-ctl HTTP server is reachable and healthy.
// Returns an error message if not available, empty string if OK.
func checkServerAvailable(baseURL string) string {
	client := &http.Client{Timeout: 5 * time.Second}
	resp, err := client.Get(baseURL + "/health")
	if err != nil {
		return fmt.Sprintf("cloud-ctl server not reachable at %s: %v", baseURL, err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Sprintf("cloud-ctl server health check failed: %v", err)
	}

	if resp.StatusCode != http.StatusOK {
		return fmt.Sprintf("cloud-ctl server unhealthy (HTTP %d): %s", resp.StatusCode, string(body))
	}
	...
	if err := json.Unmarshal(body, &health); err != nil {
		return fmt.Sprintf("cloud-ctl server health response invalid: %v", err)
	}
	if health.Status != "healthy" {
		return fmt.Sprintf("cloud-ctl server reports status: %s", health.Status)
	}
	return ""
}
...
func runAgenticLoop(ctx context.Context, client llm.Client, baseURL string, tc TestCase) (string, []ToolTrace, int, error) {
	tools := CloudTools()
	...
	for resp.StopReason == "tool_use" && rounds < maxRounds {
		...
		for _, tc := range resp.ToolCalls {
			output, isError := ExecuteToolHTTP(baseURL, tc.Name, tc.Input)
...
func TestCloudCtlHTTPToolUse(t *testing.T) {
	testConfig := loadConfig(t)

	// Gate: cloud-ctl HTTP server must be available and healthy.
	if msg := checkServerAvailable(testConfig.CloudCtlURL); msg != "" {
		t.Skipf("SKIP: %s", msg)
	}
	t.Logf("cloud-ctl server available at %s", testConfig.CloudCtlURL)
	...
	t.Logf("Running HTTP tool-use matrix: %d configured models x %d tests",
		...
		t.Run(resolvedProvider+"/"+resolvedModel, func(t *testing.T) {
			...
			for _, tc := range testMatrix {
				...
				t.Run(testName, func(t *testing.T) {
					...
					start := time.Now()
					response, traces, rounds, err := runAgenticLoop(ctx, client, testConfig.CloudCtlURL, tc)
	...
	reportFile := fmt.Sprintf("RESULTS_HTTP_TOOL_USE_%s.md", timestamp)
...
func generateReport(dir, filename string, testMatrix []TestCase, results []TestResult) error {
	...
	sb.WriteString("# Cloud-CTL HTTP Tool Use Benchmark Report\n\n")
	sb.WriteString(fmt.Sprintf("Generated: `%s`\n\n", time.Now().Format("2006-01-02 15:04:05")))

	// Tool definitions summary
	sb.WriteString("## Tool Definitions\n\n")
	sb.WriteString("| Tool | HTTP Endpoint | Description |\n")
	sb.WriteString("|---|---|---|\n")
	for _, tool := range CloudTools() {
		sb.WriteString(fmt.Sprintf("| `%s` | `%s` | %s |\n",
			tool.Name, toolToEndpoint(tool.Name), truncate(tool.Description, 60)))
	}
	sb.WriteString("\n")
...
func toolToEndpoint(name string) string {
	switch name {
	case "drive_ls":
		return "GET /api/v1/files"
	case "drive_search":
		return "GET /api/v1/files?q=Q"
	case "email_search":
		return "GET /api/v1/emails?q=Q"
	case "calendar_list":
		return "GET /api/v1/calendar/events"
	case "calendar_calendars":
		return "GET /api/v1/calendar/calendars"
	case "sheets_read":
		return "GET /api/v1/sheets/:id"
	default:
		return name
	}
}
