...
// and executes them via the real `cl` binary with --json output.
...
// =============================================================================
// Tool Executor (live via cl binary)
// =============================================================================

// ExecuteTool runs a tool call via the real `cl` binary.
// Requires `cl` in PATH with valid authentication.
func ExecuteTool(name string, input any) (string, bool) {
	...
	switch name {
	case "drive_ls":
		cmdArgs = []string{"drive", "ls", "--json", "--max", maxOrDefault(args, "max", 10)}
		if parent, ok := args["parent"].(string); ok && parent != "" {
			cmdArgs = append(cmdArgs, parent)
		}
	...
	case "drive_search":
		...
		cmdArgs = []string{"drive", "search", query, "--json", "--max", maxOrDefault(args, "max", 10)}
	...
	case "email_search":
		...
		cmdArgs = []string{"email", "search", query, "--json", "--max", maxOrDefault(args, "max", 10)}

	case "calendar_list":
		cmdArgs = []string{"calendar", "list", "--json", "--max-results", maxOrDefault(args, "max", 20)}
		...
		if calID, ok := args["calendar_id"].(string); ok && calID != "" {
			cmdArgs = append(cmdArgs, "--calendar", calID)
		}
	...
	case "sheets_read":
		...
		cmdArgs = []string{"sheets", "read", id, "--json"}

	default:
		return fmt.Sprintf(`{"error": "unknown tool: %s"}`, name), true
	}

	cmd := exec.Command("cl", cmdArgs...)
	out, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Sprintf(`{"error": "%s: %s"}`, err.Error(), strings.TrimSpace(string(out))), true
	}
	result := strings.TrimSpace(string(out))
	// Cap tool output to ~16KB to avoid overwhelming the LLM context.
	// Real email bodies and file metadata can be massive.
